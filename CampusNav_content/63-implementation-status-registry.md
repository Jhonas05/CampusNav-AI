...
| CLARA | `17`, `18`, `20`, `41`, `60` | `src/lib/claraEngine.js`, `src/contexts/ClaraContext.jsx`, `src/components/clara/*`, `src/pages/Clara.jsx` (deep-link entry) | `PARTIAL` — floating-assistant UI and the same conservative local facility matcher exist; `DEC-UI-003` moved CLARA out of a full page without changing its reply logic | No server-side model, tool orchestration, role-aware grounding, or provider endpoint; must not be called production AI | `DEFERRED` until internal services and authorization are accepted |
...
| UI registry compliance | `06`, `07`, `21`, `26`, `55` | registered pages/components under `src/pages/` and `src/components/` | `IMPLEMENTED_VERIFIED` for code structure and automated render under `DEC-UI-003` — headless-browser runs at 1440×900, 834×1112 and 390×844 reported no console errors, no horizontal overflow, a live 2D route render, 2D↔3D switching with a correctly sized canvas, floor switching, sidebar collapse, mobile drawer, CLARA open/close/persistence, and no admin navigation for guests | Screen-reader, contrast measurement, and physical-device matrix remain pending; the 3D default camera framing has not been re-evaluated against the larger canvas | Approved UI adoption follow-up QA |
